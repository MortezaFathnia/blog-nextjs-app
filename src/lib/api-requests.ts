...
export async function getAllPosts(token?: string):Promise<any>{
  ...
  const response = await fetch(`https://ffrhqp-3000.csb.app/api/blog/getAll`, {
    method: "GET",
    credentials: "include",
    headers,
  });
...
export async function createPost(token:string,post:string):Promise<any> {
  const headers: Record<string, string> = {
    "Content-Type": "application/json",
  };
  ...
  const response = await fetch(`${SERVER_ENDPOINT}/api/blog/create`, 
  {
    method: "POST",
    credentials: "include",
    headers,
